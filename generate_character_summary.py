"""
登場人物を網羅したあらすじ生成スクリプト

このスクリプトは物語から全登場人物を抽出し、
各人物の特徴と役割を含むあらすじをGPT-5.1で生成します。

生成されたあらすじは以下の用途で使用されます:
- 質問が登場人物に関するものかの判定
- 質問の中心となる人物の特定

使用方法:
    python generate_character_summary.py

出力:
    - character_summary.txt: 登場人物を網羅したあらすじ
"""

import os
import json
from pathlib import Path
from dotenv import load_dotenv
import openai

# 環境変数読み込み
load_dotenv()
api_key = os.getenv("OPENAI_API_KEY")
if not api_key:
    print("❌ OPENAI_API_KEY が設定されていません")
    exit(1)

client = openai.OpenAI(api_key=api_key)

# ===============================================
#  小説データの読み込み
# ===============================================
def load_story(filename="beast_text.json"):
    """小説データを読み込む"""
    try:
        with open(filename, "r", encoding="utf-8") as f:
            story_sections = json.load(f)

        # 全章のテキストを結合
        story_text = "\n\n".join([
            f"【{sec['section']}章】 {sec['title']}\n\n{sec['text']}"
            for sec in story_sections
        ])

        return story_text, story_sections
    except FileNotFoundError:
        print(f"❌ {filename} が見つかりません")
        exit(1)

# ===============================================
#  登場人物を網羅したあらすじを生成
# ===============================================
def generate_character_summary(story_text: str) -> str:
    """
    GPT-5.1を使って登場人物を網羅したあらすじを生成

    Args:
        story_text: 物語の全文

    Returns:
        str: 登場人物を網羅したあらすじ
    """
    print("📝 登場人物を網羅したあらすじを生成中...")
    print(f"   - 本文文字数: {len(story_text):,} 文字")
    print(f"   - GPT-5.1にリクエスト送信中...")
    print()

    prompt = f"""
以下の物語から、**すべての登場人物**を網羅したあらすじを作成してください。

本文:
{story_text}

要件:
1. **登場人物の網羅性**
   - 主要人物はもちろん、脇役や一時的に登場する人物も含める
   - 各人物の名前、立場、特徴を明記する
   - 人物間の関係性を明確にする

2. **あらすじの構成**
   - 物語の時系列に沿って展開を記述
   - 各章ごとに登場人物を整理
   - 人物の初登場時には詳しく説明する

3. **出力形式**
   以下の形式で出力してください:

   ## 登場人物一覧
   - **人物名**: 立場・特徴・役割
   （全登場人物を列挙）

   ## あらすじ（章ごと）

   ### 第1章: [章タイトル]
   - 登場人物: [この章に登場する人物]
   - 概要: [この章の内容と人物の動き]

   （各章について同様に記述）

4. **重要事項**
   - 人物名は正確に記載（誤字・表記ゆれに注意）
   - 同一人物の別名・愛称も明記
   - 人物の関係性（家族、友人、敵対など）を明確に
   - このあらすじは「人物に関する質問への回答」に使用されるため、人物情報を詳細に記述

出力はMarkdown形式で、見出しと箇条書きを活用してください。
"""

    try:
        print("🔄 GPT-5.1からの応答をストリーミング受信中...")
        print("-" * 80)

        # ストリーミングで応答を受信
        stream = client.chat.completions.create(
            model="gpt-5.1",
            messages=[
                {"role": "system", "content": "あなたは物語の登場人物を詳細に分析し、網羅的なあらすじを作成する専門家です。"},
                {"role": "user", "content": prompt}
            ],
            temperature=0.3,
            stream=True
        )

        summary = ""
        chunk_count = 0

        for chunk in stream:
            if chunk.choices[0].delta.content is not None:
                content = chunk.choices[0].delta.content
                summary += content
                print(content, end="", flush=True)
                chunk_count += 1

        print()
        print("-" * 80)
        print(f"✅ あらすじ生成完了")
        print(f"   - 受信チャンク数: {chunk_count:,}")
        print(f"   - 生成文字数: {len(summary):,} 文字")
        print()

        return summary

    except Exception as e:
        print(f"❌ エラーが発生しました: {e}")
        exit(1)

# ===============================================
#  あらすじをファイルに保存
# ===============================================
def save_summary(summary: str, output_file: str = "character_summary.txt"):
    """あらすじをテキストファイルに保存"""
    try:
        with open(output_file, "w", encoding="utf-8") as f:
            f.write("=" * 80 + "\n")
            f.write("登場人物を網羅したあらすじ\n")
            f.write("=" * 80 + "\n\n")
            f.write(summary)
            f.write("\n\n" + "=" * 80 + "\n")
            f.write(f"生成日時: {Path(output_file).stat().st_mtime}\n")
            f.write("=" * 80 + "\n")

        print(f"💾 あらすじを {output_file} に保存しました")
        print(f"   - ファイルサイズ: {Path(output_file).stat().st_size:,} バイト")

        # 文字数をカウント
        char_count = len(summary)
        print(f"   - 文字数: {char_count:,} 文字")

        return output_file

    except Exception as e:
        print(f"❌ ファイル保存エラー: {e}")
        exit(1)

# ===============================================
#  メイン処理
# ===============================================
def main():
    print("=" * 80)
    print("登場人物を網羅したあらすじ生成")
    print("=" * 80)
    print()

    # 1. 小説データを読み込み
    print("📖 小説データを読み込み中...")
    story_text, story_sections = load_story()
    print(f"   - 総章数: {len(story_sections)}")
    print(f"   - 総文字数: {len(story_text):,} 文字")
    print()

    # 2. 登場人物を網羅したあらすじを生成
    summary = generate_character_summary(story_text)

    # 3. ファイルに保存
    output_file = save_summary(summary)

    print()
    print("=" * 80)
    print("✅ 完了!")
    print("=" * 80)
    print()
    print("生成されたファイル:")
    print(f"  - {output_file}")
    print()
    print("このあらすじは以下の用途で使用できます:")
    print("  1. 質問が登場人物に関するものかの判定")
    print("  2. 質問の中心となる人物の特定")
    print()

if __name__ == "__main__":
    try:
        main()
    except KeyboardInterrupt:
        print("\n\n⚠️  ユーザーによって中断されました")
    except Exception as e:
        print(f"\n\n❌ エラーが発生しました: {e}")
        import traceback
        traceback.print_exc()
